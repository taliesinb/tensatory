...
const fmt = (v: number | null) => (v === null ? "—" : String(+v.toFixed(6)));
...
/* value readouts: "lo – hi" plus dim "(centre ±half)" */
const readout = (el: IntervalEl) => {
  const out = document.getElementById(`${el.id}v`); if (!out) return;
  const upd = () => {
    out.innerHTML = el.lo === null
      ? `<span class="dim">unset</span>`
      : `${fmt(el.lo)} – ${fmt(el.hi)} <span class="dim">(${fmt(el.center)} ±${fmt(el.width! / 2)})</span>`;
  };
...
declare global { interface Window { proto: { sliders: typeof sliders; intervals: typeof intervals; logged: typeof logged } } }
