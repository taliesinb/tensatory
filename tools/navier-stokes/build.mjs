...
const bundle = {
  ...
  manifolds: {
    exterior: { name: "heat exterior (exact)", numDims: 3, dimNames: ["x", "y", "z"] },
    core: { name: "concentrating core (leading order)", numDims: 3, dimNames: ["x", "y", "z"] },
  },
  fields: {
    /* ---------- exterior ---------- */
    "ext/angmom": {
      kind: "scalar", domain: "exterior", name: "r·u_θ (angular momentum)", codomain: "norm",
      description: "Γ(r,τ) = c∞ (r²/2)^(−h) H(4τ/r²), H(Z) = Γ(1+h)⁻¹∫₀^∞ e^(−v) v^h (1+Zv)^(−h) dv. Decreases outward (Rayleigh-unstable), which is what amplifies the paper's pulses. Nearly constant for h = 1/100: a point vortex with a τ^(−h) core.",
    ...
    "ext/swirl": {
      ...
      description: "The azimuthal velocity Γ/r of the heat exterior; ∝ 1/r, an exact solution of the azimuthal heat equation ∂ₜu = u_rr + u_r/r − u/r². Finite at every r > 0 as τ → 0.",
      data: { type: "pointwise", expr: div("g", sqrt(r2eps)), scalars: { g: "ext/angmom" } },
    },
    "ext/vorticity": {
      kind: "scalar", domain: "exterior", name: "ω_z (vorticity)",
      description: "ω_z = (1/r) ∂_r(r u_θ) = ∇Γ·(x,y,0)/r², exact symbolic derivative of the angular momentum. Negative: angular momentum decreases outward.",
    ...
    "ext/u": {
      ...
      description: "u = Γ (−y, x, 0)/r²: purely azimuthal, independent of z. Streamlines are circles.",
    ...
    "core/q": {
      ...
      description: "q(z,τ) solving q − z² q^(2h) = τ (two Newton steps from τ + z²); q ≍ τ + |z|^(1/D). The core's radial scale is √q, its axial scale q^D, its velocity scale q^(−A), A = ½ + h.",
    ...
    "core/eta": {
      ...
      description: "η = z q^(−D) ∈ (−1, 1); η = ±1 is the singular time away from the origin.",
      data: { type: "pointwise", expr: etaExpr, scalars: { q: "core/q" } },
    },
    "core/X": {
      kind: "scalar", domain: "core", name: "X (similarity radius²)", codomain: "norm",
      description: "X = r²/(2q). The leading axis profile is valid for X ≲ 4 (Λ = 1); the paper's pulse annulus would sit at larger X.",
      data: { type: "pointwise", expr: XExpr, scalars: { q: "core/q" } },
    },
    "core/u": {
      kind: "vector", domain: "core", name: "u⁽⁰⁾ (leading velocity)",
      description: "u = (v₀/2q)(x,y,0) + q^(−A−½) ϕ (−y,x,0) + q^(−A) U e_z: inward spiral near z ≈ 0 with axial outflow above and below; exactly divergence-free. Velocity scale τ^(−½−h).",
      data: overQv(P.u),
    },
    "core/speed": {
      kind: "scalar", domain: "core", name: "|u⁽⁰⁾|", codomain: "norm",
      description: "Speed of the leading core flow; isosurfaces show the shrinking column (radius ∝ √τ).",
      data: { type: "pointwise", expr: norm("u"), vectors: { u: "core/u" } },
    },
    "core/swirl": {
      kind: "scalar", domain: "core", name: "u_θ (swirl)",
      description: "u_θ = q^(−A) E, E = √(2X) ϕ*(η) f₀(X χ(η)), f₀(z) = 2J₁(√(2z))/√(2z): a Bessel (Kelvin-like) vortex core whose amplitude ϕ* peaks at the dividing layer η₀ ≈ −0.011.",
      data: overQ(mul(P.swirlRate, sqrt(r2))),
    },
    "core/uz": {
      kind: "scalar", domain: "core", name: "u_z (axial velocity)",
      description: "u_z = q^(−A) U, U = U*(η) − X Z*(η)/(2L), U* = 4η + j₀ (j₀ = 0.05 breaks the z ↦ −z symmetry on purpose). Reverses sign at larger X: the paper's radial-outflow region.",
      data: overQ(mul(P.qA, P.U)),
    },
    "core/ur": {
      kind: "scalar", domain: "core", name: "u_r (radial velocity)",
      description: "u_r = r v₀/(2q) from incompressibility, paper (4.7): negative (inflow) near the mid-plane.",
      data: overQ(mul(P.radial, sqrt(r2))),
    },
    "core/angmom": {
      kind: "scalar", domain: "core", name: "r·u_θ (angular momentum)", codomain: "norm",
      description: "r u_θ = q^(−h) H(X, η), H = 2X ϕ: transported inward by the radial inflow, the spin-up mechanism.",
      data: overQ(mul(P.swirlRate, r2)),
    },
    "core/pressure": {
      kind: "scalar", domain: "core", name: "p⁽⁰⁾ (pressure)",
      description: "p = q^(−2A) Π, Π_X = E²/(2X) integrated from the axis datum Π₀(η) = −1/(1+η²)². Low pressure in the core supplies the centripetal force.",
      data: overQ(P.p),
    },
    "core/vorticity": {
      kind: "vector", domain: "core", name: "ω = curl u⁽⁰⁾",
      description: "Vorticity in closed form (z-derivatives through the similarity map by the paper's Lemma 4.1; checked against the exact symbolic curl). Put it in a scalar slot for |ω| isosurfaces, the vortex-tube picture. The generic {op:\"grad\"} curl of the velocity tree is exact too but its own gradient (isosurface normals) is a second derivative of a 40 kB expression: minutes of shader generation.",
      data: overQv(P.omega),
    },
    "core/vortz": {
      kind: "scalar", domain: "core", name: "ω_z (axial vorticity)",
      description: "ω_z = (1/r) ∂_r(r u_θ) = q^(−A−½) ϕ* [2f₀(Xχ) + 2Xχ f₀'(Xχ)]: the Bessel core's vorticity, positive on the axis, changing sign where the swirl profile turns over.",
      data: overQ(P.omZ),
    },
  },
  pointSets: {
