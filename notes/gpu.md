...
**Resident colour sharpens progressively** (`gpu/src/recolour.ts`, viewer
`recolour.ts`). Interpolated colour is aliased on a surface, so once the level
is at rest a recolour kernel evaluates the colour field's own program at a
batch of records per frame and writes exact colours into the set's buffer in
