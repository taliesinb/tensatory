...
export interface Recolourer {
  /** recolour records phase, phase + stride, phase + 2·stride, … (`batch` of them); records beyond the live count are skipped */
  dispatch(buffer: GPUBuffer, indirect: GPUBuffer, phase: number, stride: number, batch: number): void;
}
    ...
    dispatch(buffer, indirect, phase, stride, batch) {
      backend.dispatch({
        ...
        buffers: [
          ...
          { role: "r", data: new Uint32Array([phase, stride, batch]) },
        ],
      });
    },
  };
 ...
 * starts (⌈total / share⌉), so later frames keep the same lattice of records. Returns whether anything is left.
 */
export function recolourStep(jobs: { r: Recolourer; buffer: GPUBuffer; indirect: GPUBuffer; total: number; progress: RecolourProgress }[], budget: number): boolean {
  ...
  for (const j of pending) {
    if (j.progress.frames === 0) j.progress.frames = Math.max(1, Math.ceil(j.total / share));
    const G = j.progress.frames, batch = Math.ceil(j.total / G);
    j.r.dispatch(j.buffer, j.indirect, j.progress.next, G, batch);
    j.progress.next++;
